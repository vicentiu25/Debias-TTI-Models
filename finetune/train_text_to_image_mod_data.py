...
from datasets import load_dataset, Features, Array3D, Dataset, concatenate_datasets
...
def main():
   ...
   #         )
    ...
    def tokenize_captions_pairs(captions):
        inputs = tokenizer(
            captions, max_length=tokenizer.model_max_length, padding="max_length", truncation=True, return_tensors="pt"
        )
        return inputs.input_ids
    ...
    def preprocess_train(examples):
        ...
        examples["positive_pixel_values"] = [train_transforms(image) for image in positive_images]
        ...
        examples["input_ids"] = tokenize_captions_pairs(examples['text'])
    ...
    def collate_fn(examples):
        ...
        positive_pixel_values = torch.stack([example["positive_pixel_values"] for example in examples])
        positive_pixel_values = positive_pixel_values.to(memory_format=torch.contiguous_format).float()

        negative_pixel_values = torch.stack([example["negative_pixel_values"] for example in examples])
        negative_pixel_values = negative_pixel_values.to(memory_format=torch.contiguous_format).float()

        input_ids = torch.stack([example["input_ids"] for example in examples])
        positive_ids = torch.stack([example["positive_ids"] for example in examples])
        negative_ids = torch.stack([example["negative_ids"] for example in examples])
        return {"pixel_values": pixel_values, "positive_pixel_values": positive_pixel_values, "negative_pixel_values": negative_pixel_values, "input_ids": input_ids, "positive_ids": positive_ids, "negative_ids": negative_ids}
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        train_loss = 0.0
        train_lang_loss = 0.0
        train_vision_loss = 0.0
        train_initial_loss = 0.0
        epoch_loss = 0.0
        epoch_lang_loss = 0.0
        epoch_vision_loss = 0.0
        epoch_initial_loss = 0.0
        epoch_step = 0
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(unet, text_encoder):
                ...
                if args.snr_gamma is None:
                    ...
                    lang_loss = ((positive_cos_sim_lang - negative_cos_sim_lang) ** 2).mean()
                    ...
                    #vision_loss = (positive_loss - negative_loss) ** 2
                    loss = 0.45 * initial_loss + 0.1 * lang_loss + 0.45 * vision_loss

                    #if epoch == 0:
                    #    loss = 0.5 * initial_loss + 0.05 * lang_loss + 0.45 * vision_loss
                    #elif epoch == 1:
                    #    loss = 0.4 * initial_loss + 0.05 * lang_loss + 0.55 * vision_loss
                    #elif epoch == 2:
                    #    loss = 0.25 * initial_loss + 0.1 * lang_loss + 0.65 * vision_loss
                    #elif epoch == 3:
                    #    loss = 0.1 * initial_loss + 0.15 * lang_loss + 0.75 * vision_loss
                    #else:
                    #    loss = 0.0 * initial_loss + 0.2 * lang_loss + 0.8 * vision_loss
                else:
                ...
                avg_lang_loss = accelerator.gather(lang_loss.repeat(args.train_batch_size)).mean()
                train_lang_loss += avg_lang_loss.item() / args.gradient_accumulation_steps

                avg_vision_loss = accelerator.gather(vision_loss.repeat(args.train_batch_size)).mean()
                train_vision_loss += avg_vision_loss.item() / args.gradient_accumulation_steps

                avg_initial_loss = accelerator.gather(initial_loss.repeat(args.train_batch_size)).mean()
                train_initial_loss += avg_initial_loss.item() / args.gradient_accumulation_steps
            ...
            if accelerator.sync_gradients:
                ...
                if accelerator.is_main_process:
                    ...
                    epoch_initial_loss += train_initial_loss
                    epoch_lang_loss += train_lang_loss
                    epoch_vision_loss += train_vision_loss
                    epoch_step += 1
                train_loss = 0.0
                train_initial_loss = 0.0
                train_lang_loss = 0.0
        ...
        if accelerator.is_main_process:
            ...
            epoch_initial_losses.append(epoch_initial_loss/epoch_step)
            epoch_lang_losses.append(epoch_lang_loss/epoch_step)
            epoch_vision_losses.append(epoch_vision_loss/epoch_step)
            logger.info(f"")
    ...
    if accelerator.is_main_process:
        ...
        plt.plot(range(1, len(epoch_initial_losses) + 1), epoch_initial_losses, label='Initial Loss')
        plt.plot(range(1, len(epoch_lang_losses) + 1), epoch_lang_losses, label='Language Loss')
        plt.plot(range(1, len(epoch_vision_losses) + 1), epoch_vision_losses, label='Vision Loss')
        plt.xlabel('Epoch')
        plt.xticks(range(1, len(epoch_losses) + 1))
        plt.ylabel('Loss')
        plt.title(f'Training Loss per Epoch {VERSION}')
        plt.grid(True)
        ...
        plt.savefig(f'finetuning_loss_{VERSION}.jpg', bbox_inches='tight')
